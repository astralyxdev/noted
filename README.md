...
`lease_s` — срок аренды, по умолчанию 300 секунд (`NOTED_LEASE_S`). Продлевайте `heartbeat`-ом с
интервалом заметно меньше срока. Перестали — задачу заберёт другой агент, и это правильно:
значит, вы умерли.

Если агент работает дольше аренды и молчит, задачу подберёт другой, и делать её будут двое.
Поэтому либо зовите `heartbeat`, либо берите `lease_s` с запасом на худший случай — и всегда
передавайте `assignee_id` в `set_status`, чтобы опоздавший получил `not_owner`, а не перезаписал
чужой результат молча.
